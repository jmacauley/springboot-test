...
@SpringBootTest(classes = { SubscriptionService.class, SubscriptionServiceBean.class, SubscriptionRepository.class, Subscription.class, DbUnitTestConfiguration.class })
@DataJpaTest
@ActiveProfiles("test")
public class SubscriptionServiceTest {
  ...
  public void testKeySet() {
    Assert.assertEquals(2, subscriptionService.keySet().size());

    for (String key : subscriptionService.keySet()) {
  ...
  public void testValues() {
    Subscription findOne = subscriptionService.get(DDSURL_1);
    ...
    findOne = subscriptionService.get(DDSURL_2);
  ...
  public void testFindByHref() {
    Subscription find = subscriptionService.getByHref(HREF_1);
    Assert.assertEquals(DDSURL_1, find.getDdsURL());

    find = subscriptionService.getByHref(HREF_2);
